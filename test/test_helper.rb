...
module TestHelper
  ...
  def assert_invalid_hand_of_type(klass, *card_strings)
    assert klass.can_create_hand_from?(create_poker_aware_cards(*card_strings)) != true
  end
  ...
  def create_poker_aware_cards(*card_strings)
    Game::PokerAwareCards.create(create_cards(*card_strings))
  end

  def two_pair_hand_from(*cards)
    cards = create_poker_aware_cards(*cards)
